...
import { isOpen, publicCrew, readAllCrews, sweepExpired, withInCounts } from '../lib/crews.js';
...
export async function GET(request) {
  ...
  try {
    ...
    if (wantAll) {
      const rows = all.map((c) => Object.assign(publicCrew(c), { open: isOpen(c) }));
      return Response.json({ count: rows.length, crews: rows }, { headers: { 'Cache-Control': 'no-store' } });
    }

    const now = Date.now();
    const open = all.filter((c) => isOpen(c, now)).map(publicCrew);
