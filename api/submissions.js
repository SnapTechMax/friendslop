...
export async function GET(request) {
  ...
  try {
    ...
    if (format === 'csv') {
      const cols = ['id', 'status', 'title', 'url', 'minPlayers', 'maxPlayers', 'tags', 'devName', 'email', 'onBehalf', 'credit', 'cover', 'submittedAt', 'reviewedAt', 'approvedAt', 'note'];
      const lines = [cols.join(',')];
      for (const r of rows) {
        lines.push([r.id, r.status, r.title, r.url, r.players?.min, r.players?.max, r.tags, r.devName, r.email, r.onBehalf, r.credit, r.cover, r.submittedAt, r.reviewedAt, r.approvedAt, r.note].map(csvCell).join(','));
      }
